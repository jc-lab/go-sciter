...
var (
	BAD_HWINDOW HWINDOW = 0
)

func Wrap(hwnd HWINDOW) *Sciter {
	s := &Sciter{
		hwnd:      hwnd,
...
func ClassName() string {
	api := GetApi()
	cname := (*uint16)(api.SciterClassName())
	return Utf16ToString(cname)
}

func VersionAsString() string {
	api := GetApi()
	a, b, c, d := api.SciterVersion(0), api.SciterVersion(1), api.SciterVersion(2), api.SciterVersion(3)
...
func CreateWindow(createFlags WindowCreationFlag, rect *Rect, delegate uintptr, delegateParam uintptr, parent HWINDOW) HWINDOW {
	api := GetApi()
...
func (s *Sciter) DataReady(uri string, data []byte) bool {
	...
	ret := defaultApi.SciterDataReady(s.hwnd, StringToWcharPtr(uri), pData, UINT(len(data)))
	if ret == 0 {
		return false
	}
...
//	\return \b BOOL, TRUE if Sciter accepts the data or \c FALSE if error occured
func (s *Sciter) DataReadyAsync(uri string, data []byte, requestId HREQUEST) bool {
	// args
	var pData LPCBYTE
	if len(data) > 0 {
		pData = (LPCBYTE)(unsafe.Pointer(&data[0]))
	...
	crequestId := LPVOID(requestId)

	// cgo call
	ret := defaultApi.SciterDataReadyAsync(s.hwnd, curi, pData, cdataLength, crequestId)
	if ret == 0 {
		return false
	}
	return true
...
func (s *Sciter) LoadFile(filename string) error {
	ret := defaultApi.SciterLoadFile(s.hwnd, StringToWcharPtr(filename))
	if ret == 0 {
...
func (s *Sciter) LoadHtml(html, baseUrl string) error {
	// args
	chtml := (LPCBYTE)(unsafe.Pointer(StringToBytePtr(html)))
	csize := UINT(len(html))
	cbaseUrl := StringToWcharPtr(baseUrl)
	// cgo call
	ret := defaultApi.SciterLoadHtml(s.hwnd, chtml, csize, cbaseUrl)
	if ret == 0 {
...
func (s *Sciter) SetCallback(handler *CallbackHandler) {
	...
	cparam := LPVOID(unsafe.Pointer(uintptr(idx)))
	// cgo call
	defaultApi.SciterSetCallback(s.hwnd, sciterhostcallback, cparam)
}
...
func SetMasterCSS(data string) (ok bool) {
	// args
	cutf8 := LPCBYTE(unsafe.Pointer(StringToBytePtr(data)))
	clen := UINT(len(data))
	// cgo call
	r := defaultApi.SciterSetMasterCSS(cutf8, clen)
	if r == 0 {
		return false
	}
	return true
...
func AppendMasterCSS(data string) (ok bool) {
	...
	clen := UINT(len(data))
	// cgo call
	r := defaultApi.SciterAppendMasterCSS(cutf8, clen)
	if r == 0 {
		return false
	}
	return true
...
func (s *Sciter) SetCSS(css, baseUrl, mediaType string) (ok bool) {
	// args
	ccss := LPCBYTE(unsafe.Pointer(StringToBytePtr(css)))
	clen := UINT(len(css))
	cbaseUrl := StringToWcharPtr(baseUrl)
	cmediaType := StringToWcharPtr(mediaType)
	// cgo call
	r := defaultApi.SciterSetCSS(s.hwnd, ccss, clen, cbaseUrl, cmediaType)
	if r == 0 {
		return false
	}
	return true
...
func (s *Sciter) SetMediaType(mediaType string) (ok bool) {
	// args
	cmediaType := LPCWSTR(unsafe.Pointer(StringToWcharPtr(mediaType)))
	// cgo call
	r := defaultApi.SciterSetMediaType(s.hwnd, cmediaType)
	if r == 0 {
		return false
	}
	return true
...
func (s *Sciter) MinWidth() int {
	r := defaultApi.SciterGetMinWidth(s.hwnd)
	return int(r)
}
...
func (s *Sciter) MinHeight(width int) int {
	...
	cwidth := UINT(width)
	// cgo call
	r := defaultApi.SciterGetMinHeight(s.hwnd, cwidth)
	return int(r)
}
...
func (s *Sciter) Call(functionName string, args ...*Value) (retval *Value, err error) {
	...
	// args
	funcName := CString(functionName)
	cfn := LPCSTR(unsafe.Pointer(funcName))
	cargc := UINT(argc)
	var cargv *SCITER_VALUE
	if len(argv) > 0 {
		cargv = (*SCITER_VALUE)(unsafe.Pointer(&argv[0]))
	}
	cretval := (*SCITER_VALUE)(unsafe.Pointer(retval))
	// cgo call
	r := defaultApi.SciterCall(s.hwnd, cfn, cargc, cargv, cretval)
	if r == 0 {
...
func (s *Sciter) Eval(script string) (retval *Value, ok bool) {
	...
	cscriptLength := UINT(len(u16) - 1)
	cscript := LPCWSTR(unsafe.Pointer(&u16[0]))
	cretval := (*SCITER_VALUE)(unsafe.Pointer(retval))
	// cgo call
	r := defaultApi.SciterEval(s.hwnd, cscript, cscriptLength, cretval)
	if r == 0 {
		ok = false
	} else {
		ok = true
...
func (s *Sciter) UpdateWindow() {
	defaultApi.SciterUpdateWindow(s.hwnd)
}
...
func (s *Sciter) SetOption(option Sciter_RT_OPTIONS, value uint) (ok bool) {
	// args
	coption := UINT(option)
	cvalue := UINT_PTR(value)
	// cgo call
	r := defaultApi.SciterSetOption(s.hwnd, coption, cvalue)
	if r == 0 {
		return false
	}
	return true
...
func SetOption(option Sciter_RT_OPTIONS, value uint) (ok bool) {
	...
	cvalue := UINT_PTR(value)
	hwnd := BAD_HWINDOW
	r := defaultApi.SciterSetOption(hwnd, coption, cvalue)
	if r == 0 {
		return false
	}
	return true
...
func (s *Sciter) SetHomeURL(baseUrl string) (ok bool) {
	// args
	cbaseUrl := LPCWSTR(unsafe.Pointer(StringToWcharPtr(baseUrl)))
	// cgo call
	r := defaultApi.SciterSetHomeURL(s.hwnd, cbaseUrl)
	if r == 0 {
		return false
	}
	return true
...
func (s *Sciter) OpenArchive(data []byte) {
	s.har = defaultApi.SciterOpenArchive((*BYTE)(&data[0]), UINT(len(data)))
}
...
func (s *Sciter) GetArchiveItem(uri string) []byte {
	...
	var length UINT
	cdata := (*LPCBYTE)(&data)
	r := defaultApi.SciterGetArchiveItem(s.har, StringToWcharPtr(uri), cdata, &length)
	if r == 0 {
		return nil
	}
...
func (s *Sciter) CloseArchive() {
	defaultApi.SciterCloseArchive(s.har)
	s.har = HSARCHIVE(0)
}
...
// Wrap HELEMENT to a go side *Element, doing Sciter_UseElement/Sciter_UnuseElement automatically
func WrapElement(he HELEMENT) *Element {
	e := &Element{
		handle: he,
	}
	e.use()
...
func (e *Element) use() error {
	r := defaultApi.Sciter_UseElement(e.handle)
...
func (e *Element) unUse() error {
	r := defaultApi.Sciter_UnuseElement(e.handle)
...
func (s *Sciter) GetRootElement() (*Element, error) {
	var he HELEMENT
	r := defaultApi.SciterGetRootElement(s.hwnd, &he)
...
func (s *Sciter) GetFocusElement() (*Element, error) {
	var he HELEMENT
	r := defaultApi.SciterGetFocusElement(s.hwnd, &he)
...
func (s *Sciter) FindElement(pt Point) (*Element, error) {
	var he HELEMENT
	r := defaultApi.SciterFindElement(s.hwnd, pt, &he)
...
func (e *Element) ChildrenCount() (int, error) {
	var count UINT
	r := defaultApi.SciterGetChildrenCount(e.handle, &count)
...
func (e *Element) NthChild(n int) (*Element, error) {
	var he HELEMENT
	r := defaultApi.SciterGetNthChild(e.handle, UINT(n), &he)
...
func (e *Element) ParentElement() (*Element, error) {
	var he HELEMENT
	r := defaultApi.SciterGetParentElement(e.handle, &he)
...
func goLPCSTR_RECEIVER(bs LPCSTR, n uint, param unsafe.Pointer) int {
	s := (*string)(param)
	*s = GoString(bs)
	return 0
}
...
func (e *Element) Html(outer bool) (string, error) {
	...
	cparam := LPVOID(unsafe.Pointer(&bs))
	// cgo call
	r := defaultApi.SciterGetElementHtmlCB(e.handle, couter, lpcbyte_receiver, cparam)
	str := string(bs)
...
func (e *Element) Text() (string, error) {
	var str string
	// args
	cparam := LPVOID(unsafe.Pointer(&str))
	// cgo call
	r := defaultApi.SciterGetElementTextCB(e.handle, lpcwstr_receiver, cparam)
...
func (e *Element) SetText(text string) error {
	...
	clength := UINT(len(u16) - 1)
	ctext := LPCWSTR(unsafe.Pointer(&u16[0]))
	// cgo call
	r := defaultApi.SciterSetElementText(e.handle, ctext, clength)
...
func (e *Element) AttrCount() (int, error) {
	var count UINT
	// cgo call
	r := defaultApi.SciterGetAttributeCount(e.handle, &count)
...
func (e *Element) NthAttrName(n int) (name string, err error) {
	// args
	cn := UINT(n)
	cparam := LPVOID(unsafe.Pointer(&name))
	// cgo call
	r := defaultApi.SciterGetNthAttributeNameCB(e.handle, cn, lpcstr_receiver, cparam)
...
func (e *Element) NthAttr(n int) (value string, err error) {
	// args
	cn := UINT(n)
	cparam := LPVOID(unsafe.Pointer(&value))
	// cgo call
	r := defaultApi.SciterGetNthAttributeValueCB(e.handle, cn, lpcwstr_receiver, cparam)
...
func (e *Element) Attr(name string) (string, error) {
	...
	cparam := LPVOID(unsafe.Pointer(&str))
	// cgo call
	r := defaultApi.SciterGetAttributeByNameCB(e.handle, cname, lpcwstr_receiver, cparam)
...
func (e *Element) SetAttr(name, val string) error {
	// args
	cname := CString(name)
	cval := LPCWSTR(StringToWcharPtr(val))
	// cgo call
	r := defaultApi.SciterSetAttributeByName(e.handle, cname, cval)
...
func (e *Element) ClearAttr() error {
	return wrapDomResult(defaultApi.SciterClearAttributes(e.handle), "SciterClearAttributes")
}
...
func (e *Element) Index() (idx int, err error) {
	...
	var cidx UINT
	// cgo call
	r := defaultApi.SciterGetElementIndex(e.handle, &cidx)
	idx = int(cidx)
...
func (e *Element) Type() (string, error) {
	...
	cparam := LPVOID(unsafe.Pointer(&str))
	// cgo call
	r := defaultApi.SciterGetElementTypeCB(e.handle, lpcstr_receiver, cparam)
...
func (e *Element) Style(name string) (string, error) {
	...
	cparam := LPVOID(unsafe.Pointer(&str))
	// cgo call
	r := defaultApi.SciterGetStyleAttributeCB(e.handle, cname, lpcwstr_receiver, cparam)
...
func (e *Element) SetStyle(name, val string) error {
	...
	cval := LPCWSTR(StringToWcharPtr(val))
	// cgo call
	r := defaultApi.SciterSetStyleAttribute(e.handle, cname, cval)
...
func (e *Element) ScrollToView(flag SCITER_SCROLL_FLAGS) error {
	...
	cflag := UINT(flag)
	// cgo call
	r := defaultApi.SciterScrollToView(e.handle, cflag)
...
func (e *Element) Update(forceRender bool) error {
	// args
	var cforceRender SBOOL = FALSE
	if forceRender {
		cforceRender = TRUE
	}
	// cgo call
	return wrapDomResult(defaultApi.SciterUpdateElement(e.handle, cforceRender), "SciterUpdateElement")
}
...
func (e *Element) Capture() error {
	return wrapDomResult(defaultApi.SciterSetCapture(e.handle), "SciterSetCapture")
}

// SCDOM_RESULT  SciterReleaseCapture(HELEMENT he)
func (e *Element) ReleaseCapture() error {
	return wrapDomResult(defaultApi.SciterReleaseCapture(e.handle), "SciterReleaseCapture")
}

// SCDOM_RESULT  SciterGetElementHwnd(HELEMENT he, HWINDOW* p_hwnd, BOOL rootWindow)

// Get HWINDOW of containing window.
func (e *Element) GetHwnd(rootWindow bool) (hwnd HWINDOW, err error) {
	// args
	var crootWindow SBOOL = FALSE
	if rootWindow {
		crootWindow = TRUE
	}
	// cgo call
	r := defaultApi.SciterGetElementHwnd(e.handle, &hwnd, crootWindow)
...
func (e *Element) CombineURL(url string) (combinedUrl string, err error) {
	...
	// args
	cBuffer := LPWSTR(unsafe.Pointer(&buf[0]))
	cBufferSize := UINT(len(buf))
	// cgo call
	r := defaultApi.SciterCombineURL(e.handle, cBuffer, cBufferSize)
...
func goSciterElementCallback(he HELEMENT, param unsafe.Pointer) int {
...
func (e *Element) Select(css_selectors string) ([]*Element, error) {
	...
	// args
	cselectors := LPCWSTR(StringToWcharPtr(css_selectors))
	cparam := LPVOID(unsafe.Pointer(uintptr(idx)))
	// cgo call
	r := defaultApi.SciterSelectElementsW(e.handle, cselectors, sciterElementCallback, cparam)
...
func (e *Element) SelectParent(css_selector string, depth int) (*Element, error) {
	var heFound HELEMENT
	// args
	cselector := LPCWSTR(StringToWcharPtr(css_selector))
	cdepth := UINT(depth)
	// cgo call
	r := defaultApi.SciterSelectParentW(e.handle, cselector, cdepth, &heFound)
...
func (e *Element) SetHtml(html string, where SET_ELEMENT_HTML) error {
	...
	// args
	chtml := (*BYTE)(StringToBytePtr(html))
	clen := UINT(len(html))
	cwhere := UINT(where)
	// cgo call
	r := defaultApi.SciterSetElementHtml(e.handle, chtml, clen, cwhere)
...
func (e *Element) UID() (uint, error) {
	...
	var uid UINT
	// cgo call
	r := defaultApi.SciterGetElementUID(e.handle, &uid)
...
func (s *Sciter) GetElementByUID(uid uint) (*Element, error) {
	...
	var he HELEMENT
	// cgo call
	r := defaultApi.SciterGetElementByUID(s.hwnd, cuid, &he)
...
func (e *Element) ShowPopup(eAnchor *Element, placement PopupPlacement) error {
	// args
	cplacement := UINT(placement)
	// cgo call
	r := defaultApi.SciterShowPopup(e.handle, eAnchor.handle, cplacement)
...
//	// args
//	var cpos POINT = *(*POINT)(unsafe.Pointer(&pos))
//	cplacement := UINT(placement)
//	// cgo call
//	r := defaultApi.SciterShowPopupAt(e.handle, cpos, cplacement)
...
func (e *Element) HidePopup() error {
	// cgo call
	r := defaultApi.SciterHidePopup(e.handle)
...
func (e *Element) State() (ElementState, error) {
	...
	var state UINT
	// cgo call
	r := defaultApi.SciterGetElementState(e.handle, &state)
...
func (e *Element) SetState(bitsToSet, bitsToClear ElementState, updateView bool) error {
	// args
	cbitsToSet := UINT(bitsToSet)
	cbitsToClear := UINT(bitsToClear)
	var cupdateView SBOOL = FALSE
	if updateView {
		cupdateView = TRUE
	}
	// cgo call
	r := defaultApi.SciterSetElementState(e.handle, cbitsToSet, cbitsToClear, cupdateView)
...
func CreateElement(tagname, textOrNull string) (*Element, error) {
	// args
	var he HELEMENT
	ctagname := LPCSTR(unsafe.Pointer(StringToBytePtr(tagname)))
	ctextOrNull := LPCWSTR(StringToWcharPtr(textOrNull))
	// cgo call
	r := defaultApi.SciterCreateElement(ctagname, ctextOrNull, &he)
...
func (e *Element) Clone() (*Element, error) {
	...
	var he HELEMENT
	// cgo call
	r := defaultApi.SciterCloneElement(e.handle, &he)
...
func (e *Element) Insert(el *Element, index int) error {
	...
	cindex := UINT(index)
	// cgo call
	r := defaultApi.SciterInsertElement(el.handle, e.handle, cindex)
...
func (e *Element) Detach() error {
	// cgo call
	r := defaultApi.SciterDetachElement(e.handle)
...
func (e *Element) Delete() error {
	// cgo cal
	r := defaultApi.SciterDeleteElement(e.handle)
...
func (e *Element) Clear() error {
	// cgo cal
	r := defaultApi.SciterSetElementText(e.handle, LPCWSTR(nil), UINT(0))
...
func (e *Element) SetTimer(milliseconds int) error {
	// args
	cmilliseconds := UINT(milliseconds)
	ctimeer_id := (UINT_PTR)(0)
	// cgo call
	r := defaultApi.SciterSetTimer(e.handle, cmilliseconds, ctimeer_id)
...
func goElementEventProc(tag unsafe.Pointer, he HELEMENT, evtg uint, params unsafe.Pointer) int {
...
func (e *Element) DetachEventHandler(handler *EventHandler) error {
	...
	tag := LPVOID(unsafe.Pointer(uintptr(idx)))
	// cgo call
	if ret := defaultApi.SciterDetachEventHandler(e.handle, element_event_proc, tag); SCDOM_RESULT(ret) != SCDOM_OK {
...
func (e *Element) AttachEventHandler(handler *EventHandler) error {
	...
	if ret := defaultApi.SciterAttachEventHandler(e.handle, element_event_proc, tag); SCDOM_RESULT(ret) != SCDOM_OK {
...
func (s *Sciter) AttachWindowEventHandler(handler *EventHandler) error {
	...
	r := defaultApi.SciterWindowAttachEventHandler(s.hwnd, element_event_proc, tag, HANDLE_ALL)
...
func (s *Sciter) DetachWindowEventHandler(handler *EventHandler) error {
	...
	tag := LPVOID(unsafe.Pointer(uintptr(idx)))
	ret := defaultApi.SciterWindowDetachEventHandler(s.hwnd, element_event_proc, tag)
	globalEventHandlers[idx] = nil
...
func (e *Element) Load(url string, dataType SciterResourceType) error {
	// args
	curl := LPCWSTR(StringToWcharPtr(url))
	cdataType := UINT(dataType)
	hInitiator := BAD_HELEMENT
	// cgo call
	r := defaultApi.SciterRequestElementData(e.handle, curl, cdataType, hInitiator)
...
func (e *Element) HttpRequest(url string, dataType SciterResourceType, requestType RequestType, params ...RequestParam) error {
	...
	curl := LPCWSTR(StringToWcharPtr(url))
	cdataType := UINT(dataType)
	crequestType := UINT(requestType)
	nParams := len(params)
	cnParams := UINT(nParams)
	rParams := make([]REQUEST_PARAM, nParams)
	for i := 0; i < nParams; i++ {
		rParams[i].Name = LPCWSTR(StringToWcharPtr(params[i].Name))
		rParams[i].Value = LPCWSTR(StringToWcharPtr(params[i].Value))
	}
	var crParams *REQUEST_PARAM
	if nParams > 0 {
		crParams = (*REQUEST_PARAM)(unsafe.Pointer(&rParams[0]))
	}
	// cgo call
	r := defaultApi.SciterHttpRequest(e.handle, curl, cdataType, crequestType, crParams, cnParams)
...
func (e *Element) IsVisible() bool {
	...
	var v SBOOL
	// cgo call
	defaultApi.SciterIsElementVisible(e.handle, &v)
	if v == TRUE {
		return true
	}
	return false
...
func (e *Element) IsEnabled() bool {
	...
	var v SBOOL
	// cgo call
	defaultApi.SciterIsElementEnabled(e.handle, &v)
	if v == TRUE {
		return true
	}
	return false
...
func goELEMENT_COMPARATOR(he1 unsafe.Pointer, he2 unsafe.Pointer, arg unsafe.Pointer) int {
	...
	return cmp(WrapElement(HELEMENT(he1)), WrapElement(HELEMENT(he2)))
}
...
func (e *Element) Sort(start, count int, comparator func(*Element, *Element) int) error {
	// args
	cstart := UINT(start)
	cend := UINT(start + count)
	cmpFuncParam := LPVOID(unsafe.Pointer(&comparator))
	// cgo call
	r := defaultApi.SciterSortElements(e.handle, cstart, cend, element_comparator, cmpFuncParam)
...
func (e *Element) Swap(t *Element) error {
	return wrapDomResult(defaultApi.SciterSwapElements(e.handle, t.handle), "SciterSwapElements")
}
...
func (e *Element) CallFunction(functionName string, args ...*Value) (retval *Value, err error) {
	...
	// args
	cfn := LPCSTR(unsafe.Pointer(StringToBytePtr(functionName)))
	cargc := UINT(argc)
	var cargv *SCITER_VALUE
	if len(argv) > 0 {
		cargv = (*SCITER_VALUE)(unsafe.Pointer(&argv[0]))
	...
	cretval := (*SCITER_VALUE)(unsafe.Pointer(retval))
	// cgo call
	r := defaultApi.SciterCallScriptingFunction(e.handle, cfn, cargv, cargc, cretval)
...
func (e *Element) CallMethod(methodName string, args ...*Value) (retval *Value, err error) {
	...
	// args
	cfn := LPCSTR(unsafe.Pointer(StringToBytePtr(methodName)))
	cargc := UINT(argc)
	var cargv *SCITER_VALUE
	if len(argv) > 0 {
		cargv = (*SCITER_VALUE)(unsafe.Pointer(&argv[0]))
	...
	cretval := (*SCITER_VALUE)(unsafe.Pointer(retval))
	// cgo call
	r := defaultApi.SciterCallScriptingMethod(e.handle, cfn, cargv, cargc, cretval)
...
func (e *Element) GetValue() (*Value, error) {
	// args
	v := NewValue()
	cv := (*Value)(unsafe.Pointer(v))
	// cgo call
	r := defaultApi.SciterGetValue(e.handle, cv)
...
func (e *Element) SetValue(v *Value) error {
	...
	cv := (*Value)(unsafe.Pointer(v))
	// cgo call
	r := defaultApi.SciterSetValue(e.handle, cv)
...
func (pdst *Value) init() error {
	return wrapValueResult(VALUE_RESULT(defaultApi.ValueInit((*Value)(unsafe.Pointer(pdst)))), "ValueInit")
}
...
func (pdst *Value) clear() error {
	return wrapValueResult(VALUE_RESULT(defaultApi.ValueClear((*Value)(unsafe.Pointer(pdst)))), "ValueClear")
}
...
func (pdst *Value) Compare(val *Value) error {
	pv := (*Value)(unsafe.Pointer(pdst))
	v := (*Value)(unsafe.Pointer(val))
	return wrapValueResult(VALUE_RESULT(defaultApi.ValueCompare(pv, v)), "ValueCompare")
}
...
func (pdst *Value) Copy(psrc *Value) error {
	cpdst := (*Value)(unsafe.Pointer(pdst))
	cpsrc := (*Value)(unsafe.Pointer(psrc))
	return wrapValueResult(VALUE_RESULT(defaultApi.ValueCopy(cpdst, cpsrc)), "ValueCopy")
}
...
func (pdst *Value) Isolate() error {
	cpdst := (*Value)(unsafe.Pointer(pdst))
	return wrapValueResult(VALUE_RESULT(defaultApi.ValueIsolate(cpdst)), "ValueIsolate")
}
...
func (pdst *Value) Type(pType, pUints *uint) uint {
	cpdst := (*Value)(unsafe.Pointer(pdst))
	// args
	cpType := (*UINT)(unsafe.Pointer(pType))
	cpUnits := (*UINT)(unsafe.Pointer(pUints))
	// cgo call
	return uint(defaultApi.ValueType(cpdst, cpType, cpUnits))
}
...
func (pdst *Value) stringData() string {
	...
	var pChars *uint16
	pNumChars := uint(0)
	cpChars := (*LPCWSTR)(unsafe.Pointer(&pChars))
	cpNumChars := (*UINT)(unsafe.Pointer(&pNumChars))
	// cgo call
	ret := VALUE_RESULT(defaultApi.ValueStringData(cpdst, cpChars, cpNumChars))
	if ret == HV_OK {
...
func (pdst *Value) SetString(str string, uintType ...int) error {
	sType := UINT(UT_STRING_STRING)
	if len(uintType) > 0 {
		sType = UINT(uintType[0])
	}
	cpdst := (*Value)(unsafe.Pointer(pdst))
	// args
	pStr, length := StringToUTF16PtrWithLen(str)
	chars := (LPCWSTR)(unsafe.Pointer(pStr))
	numChars := (UINT)(length)
	// cgo call
	return wrapValueResult(VALUE_RESULT(defaultApi.ValueStringDataSet(cpdst, chars, numChars, sType)), "ValueStringDataSet")
}
...
func NewSymbol(sym string) *Value {
	...
	if err := v.SetString(sym, int(UT_STRING_SYMBOL)); err != nil {
		log.Panic(err)
	}
	return v
}
...
func (pdst *Value) Int() int {
	cpdst := (*Value)(unsafe.Pointer(pdst))
	var data int = 0
	pData := (*INT)(unsafe.Pointer(&data))
	ret := VALUE_RESULT(defaultApi.ValueIntData(cpdst, pData))
	if ret == HV_OK {
		return data
	}
	return 0
...
func (pdst *Value) SetInt(data int) error {
	cpdst := (*Value)(unsafe.Pointer(pdst))
	// args
	return wrapValueResult(VALUE_RESULT(defaultApi.ValueIntDataSet(cpdst, INT(data), T_INT, 0)), "ValueIntDataSet")
}
...
func (pdst *Value) SetBool(b bool) error {
	...
	cpdst := (*Value)(unsafe.Pointer(pdst))
	// args
	return wrapValueResult(VALUE_RESULT(defaultApi.ValueIntDataSet(cpdst, INT(data), T_BOOL, 0)), "ValueIntDataSet")
}
...
func (pdst *Value) Int64() int64 {
	cpdst := (*Value)(unsafe.Pointer(pdst))
	var data int64 = 0
	pData := (*INT64)(unsafe.Pointer(&data))
	ret := VALUE_RESULT(defaultApi.ValueInt64Data(cpdst, pData))
	if ret == HV_OK {
		return data
	}
	return 0
...
func (pdst *Value) SetInt64(data int64, typ uint) error {
	cpdst := (*Value)(unsafe.Pointer(pdst))
	// args
	return wrapValueResult(VALUE_RESULT(defaultApi.ValueInt64DataSet(cpdst, INT64(data), UINT(typ), 0)), "ValueInt64DataSet")
}
...
func (pdst *Value) Float() float64 {
	cpdst := (*Value)(unsafe.Pointer(pdst))
	var data float64 = 0
	pData := (*FLOAT_VALUE)(unsafe.Pointer(&data))
	r := defaultApi.ValueFloatData(cpdst, pData)
	if r != UINT(HV_OK) {
		return 0
	}
	return data
...
func (pdst *Value) SetFloat(data float64) error {
	cpdst := (*Value)(unsafe.Pointer(pdst))
	// args
	return wrapValueResult(VALUE_RESULT(defaultApi.ValueFloatDataSet(cpdst, FLOAT_VALUE(data), UINT(T_FLOAT), 0)), "ValueFloatDataSet")
}
...
func (pdst *Value) Bytes() []byte {
	...
	var pv LPCBYTE
	var length UINT
	pBytes := (*LPCBYTE)(unsafe.Pointer(&pv))
	// cgo call
	r := defaultApi.ValueBinaryData(cpdst, pBytes, &length)
	if r != UINT(HV_OK) {
		return nil
	}
...
func (pdst *Value) SetBytes(data []byte) error {
	...
	var pBytes LPCBYTE
	if len(data) > 0 {
		pBytes = (LPCBYTE)(unsafe.Pointer(&data[0]))
	}
	nBytes := (UINT)(len(data))
	// cgo call
	r := defaultApi.ValueBinaryDataSet(cpdst, pBytes, nBytes, T_BYTES, 0)
...
func (pdst *Value) Length() int {
	...
	//args
	var n INT
	r := defaultApi.ValueElementsCount(cpdst, &n)
	if r != UINT(HV_OK) {
...
func (pdst *Value) Index(n int) *Value {
	...
	cpdst := (*Value)(unsafe.Pointer(pdst))
	cn := INT(n)
	cret := (*Value)(unsafe.Pointer(ret))
	// cgo call
	r := defaultApi.ValueNthElementValue(cpdst, cn, cret)
	if r != UINT(HV_OK) {
		return nil
	}
	return ret
...
func (pdst *Value) SetIndex(idx int, val interface{}) error {
	// args
	cpdst := (*Value)(unsafe.Pointer(pdst))
	cval := (*Value)(unsafe.Pointer(NewValue(val)))
	// cgo call
	return wrapValueResult(VALUE_RESULT(defaultApi.ValueNthElementValueSet(cpdst, INT(idx), cval)), "ValueNthElementValueSet")
}
...
func (pdst *Value) NthElementKey(idx int) (val *Value) {
	...
	// args
	cpdst := (*Value)(unsafe.Pointer(pdst))
	cval := (*Value)(unsafe.Pointer(val))
	// cgo call
	r := defaultApi.ValueNthElementKey(cpdst, INT(idx), cval)
	// ret
	if r != UINT(HV_OK) {
		return nil
	}
...
func goKeyValueCallback(param unsafe.Pointer, key, value *Value) uintptr {
	fn := *(*KeyValueCallback)(param)
	return boolToUintptr(fn(key, value))
}
...
func (pdst *Value) EnumerateKeyValue(fn KeyValueCallback) error {
	// args
	cpval := (*Value)(unsafe.Pointer(pdst))
	cparam := LPVOID(unsafe.Pointer(&fn))
	// cgo call
	return wrapValueResult(VALUE_RESULT(defaultApi.ValueEnumElements(cpval, key_value_callback, cparam)), "ValueEnumElements")
}
...
func (pdst *Value) Set(key string, val interface{}) error {
	...
	cval := (*Value)(unsafe.Pointer(NewValue(val)))
	vkey := NewValue(key)
	ckey := (*Value)(unsafe.Pointer(vkey))
	// cgo call
	return wrapValueResult(VALUE_RESULT(defaultApi.ValueSetValueToKey(cpdst, ckey, cval)), "ValueSetValueToKey")
}
...
func (pdst *Value) Get(key string) (val *Value) {
	...
	ckey := (*Value)(unsafe.Pointer(vkey))
	// cgo call
	r := defaultApi.ValueGetValueOfKey(cpdst, ckey, cval)
	// ret
	if r != UINT(HV_OK) {
		return nil
	}
...
func (pdst *Value) ConvertToString(how ValueStringConvertType) error {
	...
	//args
	chow := UINT(how)
	return wrapValueResult(VALUE_RESULT(defaultApi.ValueToString(cpdst, chow)), "ValueToString")
}
...
func (pdst *Value) ConvertFromString(str string, how ValueStringConvertType) error {
	cval := (*Value)(unsafe.Pointer(pdst))
	// args
	...
	clen := UINT(len(u16) - 1)
	cstr := LPCWSTR(unsafe.Pointer(&u16[0]))
	chow := UINT(how)
	// cgo call
	return wrapValueResult(VALUE_RESULT(defaultApi.ValueFromString(cval, cstr, clen, chow)), "ValueFromString")
}
...
func (v *Value) Invoke(self *Value, nameOrUrl string, args ...*Value) (retval *Value, err error) {
	...
	// args
	cv := (*SCITER_VALUE)(unsafe.Pointer(v))
	cself := (*SCITER_VALUE)(unsafe.Pointer(self))
	cargc := UINT(argc)
	var cargv *SCITER_VALUE
	if len(argv) > 0 {
		cargv = (*SCITER_VALUE)(unsafe.Pointer(&argv[0]))
	...
	cretval := (*SCITER_VALUE)(unsafe.Pointer(retval))
	cnameOrUrl := LPCWSTR(StringToWcharPtr(nameOrUrl))
	// cgo call
	r := defaultApi.ValueInvoke(cv, cself, cargc, cargv, cretval, cnameOrUrl)
...
// UINT  ValueNativeFunctorSet (defaultApi.Value* pval, NATIVE_FUNCTOR_INVOKE*  pinvoke, NATIVE_FUNCTOR_RELEASE* prelease, VOID* tag )
...
func (pdst *Value) SetNativeFunctor(nf NativeFunctor) error {
	...
	cval := (*Value)(unsafe.Pointer(pdst))
	// cgo call
	r := defaultApi.ValueNativeFunctorSet(cval, pinvoke, prelease, uintptr(tag))
...
func (pdst *Value) IsNativeFunctor() bool {
	cval := (*Value)(unsafe.Pointer(pdst))
	r := defaultApi.ValueIsNativeFunctor(cval)
	if r == 0 {
		return false
	}
	return true
